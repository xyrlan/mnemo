...
def _no_real_detached_jobs(request: pytest.FixtureRequest, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # #426: the child's report card and its checks watch run in a detached
    # `mnemo child-report` that polls `gh` for up to half an hour.
    monkeypatch.setattr("mnemo.hooks.session_end._spawn_detached_child_report", lambda *a, **k: None)
