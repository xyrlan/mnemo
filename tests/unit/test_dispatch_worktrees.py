...
def test_a_spawn_whose_jobs_entry_is_missing_warns(repo: Path, monkeypatch) -> None:
    ...
    """
    monkeypatch.setattr(dispatch, "spawn_child", lambda prompt, *, cwd, model=None, lean=True, effort=None, read_only=False: "a1b2c3d4")
    monkeypatch.setattr(dispatch.claude_cli, "claude_version", lambda: "9.9.9")
    fetch = lambda issue, *, repo_root: dispatch.Issue(issue, "t", "b")  # noqa: E731
...
def test_one_bad_issue_does_not_strand_the_others(repo: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(dispatch, "spawn_child", lambda prompt, *, cwd, model=None, lean=True, effort=None, read_only=False: "aaaa1111")
...
def _boom(prompt, *, cwd, model=None, lean=True, effort=None, read_only=False):
...
def test_one_failing_piece_does_not_strand_the_others(repo: Path, monkeypatch) -> None:
    ...
    def flaky(prompt, *, cwd, model=None, lean=True, effort=None, read_only=False):
        calls["n"] += 1
        if calls["n"] == 1:
            raise dispatch.DispatchError("boom")
        return "id2"
...
def test_rollback_deletes_the_branch_so_the_retry_can_run(repo: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(dispatch, "spawn_child", lambda prompt, *, cwd, model=None, lean=True, effort=None, read_only=False: "ok1")
