...
def test_claude_registers_in_claude_json_and_project_mcp_json(tmp_home: Path, tmp_path: Path):
    ...
    assert "mnemo" in json.loads((tmp_home / ".claude.json").read_text(encoding="utf-8"))["mcpServers"]

    r2 = host.register_mcp(project=True, cwd=tmp_path)
    assert Path(r2.path) == tmp_path / ".mcp.json"
    assert "mnemo" in json.loads((tmp_path / ".mcp.json").read_text(encoding="utf-8"))["mcpServers"]

    host.unregister_mcp(project=False, cwd=tmp_path)
    assert "mnemo" not in json.loads((tmp_home / ".claude.json").read_text(encoding="utf-8")).get("mcpServers", {})
