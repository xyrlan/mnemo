...
- **consumes:** `FrictionRecord` from ledger, `LEDGER_NAME` from ledger
- **may:** pr
...
- **consumes:** `FrictionRecord` from ledger, `record(vault_root, rec) -> str | None` from ledger, `iter_records(vault_root, *, project=None, since=None) -> Iterator[FrictionRecord]` from ledger, `rank(vault_root, correction, *, project) -> list[Candidate]` from link, `resolve(correction, candidates, *, runner=None) -> LinkResult` from link
- **may:** pr
...
- **consumes:** `FrictionRecord` from ledger, `iter_records(vault_root, *, project=None, since=None) -> Iterator[FrictionRecord]` from ledger
- **may:** nothing

The only piece that writes to a vault page. Ships **inert**: automatic
