...
| `src/mnemo/core/rule_activation.py` | `_is_universal` helper; `build_index` v2 (rules table, by_project, universal, expanded to feedback+user+reference, optional `universal_threshold` kwarg); `INDEX_VERSION=2`; match helpers read new layout; `iter_enforce_rules_for_project` + `iter_enrich_rules_for_project` helpers for consumers |
...
| `src/mnemo/hooks/session_start.py` | New `_build_injection_payload` reads index, applies top-N cap, emits structured envelope; rebuild guard also triggers on `injection.enabled` |
| `src/mnemo/core/extract/promote.py` | `_render_project_page` emits `runtime: false` |
| `src/mnemo/cli.py` | Status count + doctor fidelity check + doctor enrich iteration migrated from `enforce_by_project`/`enrich_by_project` to v2 layout; new `_doctor_check_universal_promotion` |
| `src/mnemo/statusline.py` | Reads enforce/enrich rule counts via v2 iterators |
...
| `tests/unit/test_cli_activation.py`, `test_cli_status_doctor.py`, `tests/integration/test_extract_pipeline.py`, `test_session_start_activation.py`, `test_rule_activation_e2e.py` | Synthesizers and assertions migrated from v1 (`enforce_by_project`) to v2 layout in Task 5b |

---

## Task 0: Baseline — record the green test count
...
Note the "X passed" total in a scratch note (or directly into the PR body at Task 16). This is the regression guardrail: if a later task drops the count without a documented trade-off, STOP and diagnose before proceeding.
...
```
pytest tests/unit/test_rule_activation_index.py -k "build_index_v2 or universal_threshold_kwarg" -v
```

Expected: FAIL — `"rules"` key missing in returned dict; `build_index` does not accept the kwarg.
...
## Task 5a: Migrate match helpers to v2 layout; bump `INDEX_VERSION` (legacy tables kept)

**Scope narrowing**: this task migrates ONLY the match helpers and bumps the version. Legacy `enforce_by_project` / `enrich_by_project` tables stay in `build_index` output — they survive until **Task 5b** (inserted between Task 11 and Task 12), which migrates every remaining consumer (cli.py, statusline.py, 7 test files) and drops the projections in a single coordinated commit. Keeping the legacy tables here means cli.py and statusline.py keep working across Tasks 5a–11 without any interim breakage.

**Files:**
- Modify: `src/mnemo/core/rule_activation.py:33` (`INDEX_VERSION`), `match_bash_enforce` (lines 521-563), `match_path_enrich` (lines 667-694)
- Test: `tests/unit/test_rule_activation_match.py`, `tests/unit/test_rule_activation_index.py`
...
The `_write_rule` helper lives in `tests/unit/test_rule_activation_index.py:24-67` and accepts `enforce=` / `activates_on=` kwargs. Import it rather than redefining — `test_rule_activation_match.py` does not currently define its own copy.

- [ ] **Step 2: Run test to verify it fails**

```
pytest tests/unit/test_rule_activation_match.py -k "v2_layout or index_version or keeps_legacy" -v
```

Expected: FAIL — `INDEX_VERSION` is still `1`; `match_bash_enforce` still iterates `enforce_by_project` bucket by project, not the v2 rules table.

- [ ] **Step 3: Migrate match helpers (keep legacy tables intact)**
...
4. In `build_index`, **do NOT remove** the `enforce_by_project` / `enrich_by_project` emission. They stay populated exactly as before. Task 5b handles the drop.
...
Expected: all PASS. Legacy tests that inspect `idx["enforce_by_project"]` continue to pass because the projections are intact.

- [ ] **Step 5: Full test suite to confirm zero regression**

```
pytest tests/ --tb=short
```

Expected: count equals the Task 0 baseline. If anything fails, STOP — Task 5a is meant to be non-invasive for consumers.

- [ ] **Step 6: Commit**

```
git add src/mnemo/core/rule_activation.py tests/unit/test_rule_activation_match.py
git commit -m "feat(rule_activation): match helpers read v2 layout; INDEX_VERSION=2 (5a)"
```
...
## Task 5b: Migrate remaining consumers + drop legacy projections

**Scope**: at this point, Tasks 5a–11 have shipped the v2 index and payload builder while keeping the legacy `enforce_by_project` / `enrich_by_project` tables alive. Task 5b migrates every remaining caller to v2 and finally drops the projections from `build_index`. This is the task that closes C1 (blast radius) from the plan review.

**Files:**
- Modify: `src/mnemo/core/rule_activation.py` — add `iter_enforce_rules_for_project` + `iter_enrich_rules_for_project` helpers; drop the `enforce_by_project` / `enrich_by_project` emission from `build_index`
- Modify: `src/mnemo/cli.py:371-375, 557-563, 598-604` (status count, doctor fidelity, doctor enrich iteration)
- Modify: `src/mnemo/statusline.py:114-116`
- Modify: `tests/unit/test_rule_activation_match.py:21-29` (`_make_index` helper — rewrite for v2 shape) plus the two assertions at lines 511, 535-536
- Modify: `tests/unit/test_rule_activation_index.py` — migrate ~15 assertions from `enforce_by_project` / `enrich_by_project` to `rules` / `by_project`
- Modify: `tests/unit/test_cli_activation.py:24,25,582,583` — synthesizers
- Modify: `tests/unit/test_cli_status_doctor.py:436,437,478,481,498,499` — synthesizers
- Modify: `tests/integration/test_extract_pipeline.py:193-205` — assertions
- Modify: `tests/integration/test_session_start_activation.py:132,162` — assertions
- Modify: `tests/integration/test_rule_activation_e2e.py:273-275` — assertions

- [ ] **Step 1: Write the iterator-helpers failing test**
...
def test_iter_enforce_rules_for_project_includes_universal(tmp_vault):
    ...
    assert "uni-enf" in slugs_beta          # universal reaches beta
    assert "local-enf" not in slugs_beta    # local to alpha
...
```
pytest tests/unit/test_rule_activation_index.py -k iter_enforce_rules -v
```

Expected: FAIL — `ImportError: cannot import name 'iter_enforce_rules_for_project'`.

- [ ] **Step 3: Implement the two iterators**

Add to `src/mnemo/core/rule_activation.py` (after `match_path_enrich`, before `log_denial`):
...
def iter_enforce_rules_for_project(index: dict, project: str):
    ...
    Returned dicts are the per-slug rule entries from ``index["rules"]``
    (not the legacy enforce_by_project shape).
    """
    rules_table = index.get("rules", {})
    local_slugs = index.get("by_project", {}).get(project, {}).get("local_slugs", [])
    universal_slugs = index.get("universal", {}).get("slugs", [])
...
- [ ] **Step 4: Migrate `cli.py` status counts (line 371-375)**

Confirm location:
```
grep -n "Per-project rule counts" src/mnemo/cli.py
```
...
- [ ] **Step 5: Migrate `cli.py` doctor fidelity check (line 557-563)**

Replace the indexed_slugs loop:
...
- [ ] **Step 6: Migrate `cli.py` doctor enrich-iteration (line ~598)**

Locate with:
```
grep -n 'enrich_by_project' src/mnemo/cli.py
```

Replace the iteration at the remaining match line. The pattern:
...
Adjust the surrounding variable names (`rule` → `activates`) in the rest of that block — `path_globs` / `tools` now come from the `activates_on` sub-dict inside the rule, not the flat enrich entry.

- [ ] **Step 7: Migrate `statusline.py` (lines 114-116)**
...
- [ ] **Step 8: Sanity-check session_start.py already migrated**

Task 10 rewrote `_build_injection_payload`. Confirm no residual references:

```
grep -n 'enforce_by_project\|enrich_by_project' src/mnemo/hooks/session_start.py
```

Expected: no matches. If any remain, it's leftover from an incomplete Task 10 — fix before proceeding.

- [ ] **Step 9: Drop the legacy projections from `build_index`**

Edit `src/mnemo/core/rule_activation.py` `build_index`:

1. Delete the two local declarations at the top of the function:
```python
    enforce_by_project: dict[str, list[dict]] = {}
    enrich_by_project: dict[str, list[dict]] = {}
```

2. Delete the two append calls inside the file loop:
```python
                    for proj in projects:
                        enforce_by_project.setdefault(proj, []).append(enforce_entry)
```
and the symmetric enrich version. The data is already captured inside `rules[slug].enforce` / `rules[slug].activates_on`.

3. Remove the two keys from the return dict:
```python
        "enforce_by_project": enforce_by_project,
        "enrich_by_project": enrich_by_project,
```

- [ ] **Step 10: Migrate test synthesizers file-by-file**

For each of the files listed below, rewrite any hand-rolled v1 index into v2 shape, then run that file's tests before moving on. Prefer building through `build_index` over a synthetic vault when possible — it exercises real code. Use a hand-rolled v2 dict only when the test specifically checks the index shape itself.

Minimal v2 `_make_index` template for tests that synthesize directly:
...
1. `tests/unit/test_rule_activation_match.py:21-29` — replace `_make_index` with the v2 version above. Update the two assertions at lines 511 and 535-536 that look for `enrich_by_project` — they should now check `rules[slug].activates_on is None` and the `malformed` list.
2. `tests/unit/test_rule_activation_index.py` — search/replace `enforce_by_project` and `enrich_by_project` assertions with `rules` / `by_project` equivalents. For example: `"mnemo" not in index["enforce_by_project"]` becomes `all(not rule.get("enforce") for rule in index["rules"].values() if "mnemo" in rule.get("projects", []))` — OR more simply, switch to asserting via `rules[slug]["enforce"] is None`.
3. `tests/unit/test_cli_activation.py:24,25,582,583` — synthesizer uses v2 shape.
4. `tests/unit/test_cli_status_doctor.py:436,437,478,481,498,499` — same.
5. `tests/integration/test_extract_pipeline.py:193-205` — `enforce_by_project = index.get("enforce_by_project") or {}` becomes `rules_with_enforce = [s for s, r in index["rules"].items() if r.get("enforce")]`; assertions change shape accordingly.
6. `tests/integration/test_session_start_activation.py:132,162` — `"project-a" in idx["enforce_by_project"]` becomes `"project-a" in idx["by_project"]` with an additional check that at least one of the project's slugs carries an enforce block.
7. `tests/integration/test_rule_activation_e2e.py:273-275` — same pattern as above.

After each file: `pytest <that-file> -v`. Only move on when green.

- [ ] **Step 11: Full suite sanity — must match Task 0 baseline**

```
pytest tests/ --tb=short
```

Expected: count equals or exceeds the Task 0 baseline (counting the new tests added throughout the plan). If anything regressed, STOP and fix before committing.

- [ ] **Step 12: Commit**

```
git add src/mnemo/core/rule_activation.py src/mnemo/cli.py src/mnemo/statusline.py tests/
git commit -m "feat(rule_activation): drop legacy projections; migrate cli/statusline/tests to v2 (5b)"
```
...
**Files:**
- Modify: `src/mnemo/cli.py` (add `_doctor_check_universal_promotion` alongside the existing `_doctor_check_*` functions at lines 437-443; wire it into the `doctor` command at line 427)
- Test: `tests/unit/test_cli_status_doctor.py` (extend — this is the existing doctor test file)

**Convention to follow**: every existing `_doctor_check_*` (e.g. `_doctor_check_zero_hit` at cli.py:482, `_doctor_check_activation_fidelity` at cli.py:533) returns `bool` and is invoked from the `doctor` command handler. Mirror that signature.

- [ ] **Step 1: Write the failing test**

Append to `tests/unit/test_cli_status_doctor.py` (follow the existing style — see `test_doctor_runs_preflight_and_reports` at line 36 for the pattern: `tmp_home` fixture + `cli.main(["doctor"])` + `capsys.readouterr()`):
...
```
pytest tests/unit/test_cli_status_doctor.py -k universal_promotion -v
```

Expected: FAIL — the "Universal promotion health" line does not appear in doctor output.

- [ ] **Step 3: Add the check alongside peers**

Add to `src/mnemo/cli.py` after `_doctor_check_rule_integrity` (around line 630, before the doctor command handler):
...
Wire it into the doctor command at cli.py:427-444. The existing block runs seven checks:
...
Append one line after `rule_integrity_ok`:

```python
    _doctor_check_universal_promotion(vault)
```

(Return value ignored — informational check.)
...
- **C2 (two scoping mechanisms)** → Tasks 3, 4, 5a (unified index) + 7-11 (all consumers read from it) + 5b (remaining consumers + legacy drop)
...
- **Testing baseline** → Task 0 (green-count guardrail)
- **Testing (golden regression)** → Task 15
- **Docs + CHANGELOG (+ MCP fallback note)** → Task 14
- **Doctor check** → Task 13
- **Migration / rebuild guard** → Task 11 (+ `.git` fixture fix from plan review W1)

No spec section is unclaimed; W3 of the spec (LLM consolidation contract) is explicitly out of scope and has no task.

## Plan-review findings addressed

- **C1 (blast radius of legacy-table removal)** → Task 5 split into 5a (match-helper migration only, legacy tables kept) and 5b (consumer migration + legacy drop), inserted between Task 11 and Task 12. 9 files across src/ and tests/ enumerated explicitly.
- **C2 (Task 13 doctor test mismatched convention)** → Task 13 now uses `tmp_home` + `cli.main(["doctor"])` + `capsys`, mirrors existing `_doctor_check_*` signature (`-> bool`), wires into the existing check block at cli.py:437-444.
- **W1 (Task 11 missing `.git` fixture)** → fixed: adds `(tmp_vault / ".git").mkdir()` before the hook invocation.
- **W2 (Task 5→10 regression window)** → resolved by the 5a/5b split: 5a keeps legacy alive, Task 10 rewrites the hook to v2, 5b drops legacy. No intermediate dev-window regression.
- **W3 (Task 3 user/reference coverage untested)** → Task 3 adds `test_build_index_v2_covers_user_and_reference_types`.
- **W4 (Task 3 coupling to load_config)** → `build_index(vault_root, *, universal_threshold=None)`; tests pass explicit values, production keeps reading config.
- **S1 (baseline task)** → Task 0 records green count as a guardrail.
- **S2 (shared iterator helpers)** → `iter_enforce_rules_for_project` + `iter_enrich_rules_for_project` introduced in Task 5b, consumed by cli.py and statusline.py.
- **S3 (MCP fallback note in CHANGELOG)** → Task 14 CHANGELOG includes an explicit "Note on MCP fallback" section.
