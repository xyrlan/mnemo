...
- **Apply-time.** `inbox/dedup.py` gains `_detect_similar_existing(page, state, vault_root)`: weighted Jaccard over stemmed tokens of `name` (×3), `description` (×2), `body` (×1) against every existing page of the same type, no source-set requirement. **Threshold amended 2026-09-02 after measuring the real vault:** a single 0.5 cutoff never fires (whole-vault max 0.4978, true duplicates at 0.32–0.42 interleaved with false pairs); shipped as a two-signal gate — weighted Jaccard ≥ 0.32 AND name-stem overlap ≥ 0.27 (stop words removed) — giving 38 redirects and 2 false merges on 1,436 pages. Dismissed slugs are never redirect targets. The existing `_detect_drift_slug` and `_detect_stem_collision` remain and run first. On match the page's slug is redirected to the existing one and `union_with_prior_sources` accumulates the new source, so `source_count` grows and `universal_promotion` can fire.
- Tests must include a mutation guard: with either threshold set to 1.01 the family stays split; at the shipped values it merges. Fixtures must use paraphrased bodies, not identical ones (identical bodies score ~2× real pages).
