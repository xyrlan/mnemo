...
FEEDBACK_SYSTEM_PROMPT = (
    ...
    "it — one of the page's own source_files>\"}`. Copy the quote "
    "character-for-character; a quote that does not appear in the cited "
    "briefing is discarded by a mechanical check and the page is demoted. If "
    "no input file carries a user quote that supports the rule, do NOT emit it "
    "as feedback: emit it with `type: reference` and `evidence: null` — it is "
    "knowledge, not a correction.\n\n"
    "## Slug reuse\n\n"
    ...
    "Separately, if the rule is about one or a few specific files — the "
    "source names them (e.g. \"`prisma/schema.prisma` needs a migration for "
    "every new column\") — you MAY emit an `activates_on` object: the rule is "
    "then shown when a session opens or edits one of those files. Shape: "
    "`{\"tools\": [\"Edit\", \"Write\", \"MultiEdit\"], "
    "\"path_globs\": [\"<path1>\", \"<path2>\"]}`. Each entry must name a "
    "file, relative to the repo root: `src/lib/push.ts`, or "
    "`**/screens/HomeScreen.tsx` when the directory above it is unknown. "
    "Globs that name an area — `src/components/modals/**`, `**/*.tsx` — never "
    "fire and are ignored. When the rule is about a pattern, an area, or "
    "general style rather than a named file, omit `activates_on`.\n\n"
