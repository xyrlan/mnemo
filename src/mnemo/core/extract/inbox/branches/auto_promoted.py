...
# Compatibility re-export — existing tests (PR #86) and any other callers
# inside the package can keep importing ``_union_with_prior_sources`` from
# this module. The implementation lives in ``inbox.sources`` so the new
# universal-promotion branch can share it without an inter-branch import.
_union_with_prior_sources = union_with_prior_sources
